...
export default async function DocumentsPage({ params, searchParams }: {
  ...
  return <section className="org-documents-page">
    ...
    {summary && <section className="org-documents-toolbar" aria-label="Document usage">{canManage && <><strong>ใช้ไป {summary.used}/{summary.limit} เอกสาร</strong>{summary.warning && <span role="status">ใกล้เต็ม ({summary.warning}%)</span>}</>}<span>พร้อมใช้ {summary.available}</span><span>กำลังตรวจ {summary.checking}</span><span>ปฏิเสธ {summary.rejected}</span>{canManage && <span>รีเซ็ต {new Intl.DateTimeFormat("th-TH", { dateStyle: "medium" }).format(new Date(summary.reset_at))}</span>}</section>}
    ...
    <div className="work-layout">
      ...
      <div className="work-stack"><Card className="work-panel"><h2>ส่งเอกสารใหม่</h2><UploadForm organization={organization} /></Card>{canManage && <Card className="work-panel"><h2>นำเข้าจาก Google Drive</h2><DriveImportForm organization={organization} /></Card>}</div>
    </div>
  </section>;
}
