...
export default async function DocumentsPage({ params, searchParams }: {
  ...
  return <section className="mx-auto max-w-[1270px]">
    ...
    <div className="work-layout">
      <div className="work-stack"><section className="rounded-[1.2rem] border border-line bg-surface p-5 shadow-panel" aria-labelledby="documents-heading"><h2 id="documents-heading">รายการเอกสาร</h2>
        {page.documents.length === 0 ? <p className="empty-state mt-4">ยังไม่มีเอกสารที่คุณเข้าถึงได้</p> : <div className="task-list mt-4">{page.documents.map((doc) => <div key={doc.id} className="notification-card"><div className="min-w-0"><strong className="break-words"><Link href={`/o/${encodeURIComponent(organization)}/documents/${encodeURIComponent(doc.id)}`}>{doc.filename}</Link></strong><p className="field-help">{doc.status} · {doc.source_channel || "ไม่ทราบช่องทาง"} · {Math.ceil(doc.size / 1024)} KB</p></div><div className="card-actions document-actions">{doc.status !== "Trash" && <a className="secondary-button self-start" href={`/api/o/${encodeURIComponent(organization)}/documents/${encodeURIComponent(doc.id)}/original`}>ดาวน์โหลด</a>}{canManage && doc.status === "Available" && <form action={changeStatus.bind(null, organization, doc.id, "archive")}><button className="secondary-button" type="submit">เก็บถาวร</button></form>}{canManage && doc.status !== "Trash" && <form className="flex flex-col items-start gap-1" action={changeStatus.bind(null, organization, doc.id, "trash")}><button className="danger-button document-trash-button" type="submit">ย้ายไปถังขยะ</button><span className="text-xs text-muted">กู้คืนได้ภายใน 30 วันหลังย้าย</span></form>}{canManage && doc.status === "Trash" && <form action={changeStatus.bind(null, organization, doc.id, "restore")}><button className="secondary-button" type="submit">กู้คืน</button></form>}</div></div>)}</div>}
