...
type Catalog = { plans: PlanDefinition[]; billing_enabled: boolean; billing_test_mode: boolean };
...
export default async function BillingPage({ params, searchParams }: { params: Promise<{ organization: string }>; searchParams: Promise<{ plan?: string; interval?: string; payment?: string; error?: string; updated?: string }> }) {
  ...
  return <section className="workspace-page billing-page">
    ...
    {catalog.billing_test_mode && <p className="notice" role="status">โหมดทดสอบ Omise: QR นี้ไม่รับเงินจริง ให้จำลองผลสำเร็จหรือไม่สำเร็จใน Omise Dashboard</p>}
    {query.payment && pending && <p className="notice" role="status">สร้าง QR แล้ว สิทธิ์จะเปลี่ยนหลังผู้ให้บริการยืนยันการชำระเท่านั้น</p>}
