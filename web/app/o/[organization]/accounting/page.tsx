...
export default async function AccountingReviewQueue({ params }: { params: Promise<{ organization: string }> }) {
  const { organization } = await params;
  const base = `/o/${encodeURIComponent(organization)}`;
  const response = await sessionGET(`/v1${base}/accounting/review-queue`);
  ...
  const { items, limit } = await response.json() as { items: Item[]; limit: number };
