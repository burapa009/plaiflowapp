...
func (s *Store) CompleteOCR(ctx context.Context, c job.LeaseCommand, worker string, result ocr.Result, key, sha string, size int64) (string, error) {
	...
	var status string
	if err = tx.QueryRow(ctx, `SELECT status FROM documents WHERE id=$1 AND organization_id=$2 FOR UPDATE`, in.DocumentID, in.OrganizationID).Scan(&status); err != nil {
		return "", err
	}
	if status == "Trash" || status == "Purged" {
 ...
 FROM durable_jobs j JOIN document_ocr_runs o ON o.job_id=j.id WHERE j.id=$1 FOR UPDATE OF j,o`, c.JobID, c.AttemptID, leaseHash(c.LeaseToken), c.Now, worker, hex.EncodeToString(leaseHash(c.LeaseToken))).Scan(&status, &savedKey, &valid)
	...
	if size < 1 || size > ocr.MaxResultBytes {
		return "", errors.New("invalid OCR artifact")
	}
	_, err = tx.Exec(ctx, `UPDATE document_ocr_runs SET superseded_at=$3 WHERE organization_id=$1 AND document_id=$2 AND published_at IS NOT NULL AND superseded_at IS NULL AND deleted_at IS NULL`, in.OrganizationID, in.DocumentID, c.Now)
	if err != nil {
		return "", err
	}
	...
	_, err = tx.Exec(ctx, `UPDATE durable_jobs SET status='Completed',result=$2,completed_at=$3,current_attempt_id=NULL,lease_token_hash=NULL,lease_expires_at=NULL,worker_id=NULL WHERE id=$1`, c.JobID, meta, c.Now)
	if err != nil {
		return "", err
	}
	_, err = tx.Exec(ctx, `INSERT INTO audit_events(organization_id,event_type,target_type,target_id,outcome,occurred_at) VALUES($1,'ocr.completed','document',$2,'succeeded',$3)`, in.OrganizationID, in.DocumentID, c.Now)
	if err != nil {
		return "", err
	}
...
func (s *Store) OCRState(ctx context.Context, user, org, doc string) (ocr.State, error) {
	d, err := s.GetDocument(ctx, user, org, doc)
	if err != nil {
		return ocr.State{}, err
	}
	if d.Status == "Trash" || d.Status == "Purged" {
...
func (s *Store) RetryOCR(ctx context.Context, user, org, doc string, now time.Time) (ocr.State, error) {
	...
	if previous != "" && (status == "Queued" || status == "Running" || status == "Completed") {
		return ocr.State{JobID: previous, Status: status}, nil
	}
	id := postgresUUID()
	payload, _ := json.Marshal(map[string]string{"document_id": doc, "sha256": sha, "model_version": model, "preprocessing_version": pre})
	_, err = tx.Exec(ctx, `INSERT INTO durable_jobs(id,organization_id,requester_user_id,kind,status,payload,idempotency_key,max_attempts,available_at,created_at) VALUES($1,$2,$3,'ocr','Queued',$4,$5,3,$6,$6)`, id, org, user, payload, id, now)
	if err != nil {
		return ocr.State{}, err
	}
	_, err = tx.Exec(ctx, `INSERT INTO document_ocr_runs(job_id,organization_id,document_id,fingerprint,retry_of) VALUES($1,$2,$3,$4,nullif($5,'')::uuid)`, id, org, doc, fp, previous)
	if err != nil {
		return ocr.State{}, err
	}
