...
func (r Result) Validate(input Input) error {
	if r.SchemaVersion != 1 || r.InputSHA256 != input.SHA256 || r.ModelVersion != input.ModelVersion || r.PreprocessingVersion != input.PreprocessingVersion || len(r.Pages) < 1 || len(r.Pages) > 20 || r.DurationMS < 0 || r.DurationMS > 900000 {
